...
alter table "public"."assessments" add column "config_comprehension_depth" text not null default 'conceptual'::text;

alter table "public"."assessments" add constraint "assessments_config_comprehension_depth_check" CHECK ((config_comprehension_depth = ANY (ARRAY['conceptual'::text, 'detailed'::text]))) not valid;

alter table "public"."assessments" validate constraint "assessments_config_comprehension_depth_check";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.create_fcs_assessment(p_id uuid, p_org_id uuid, p_repository_id uuid, p_feature_name text, p_feature_description text, p_config_enforcement_mode text, p_config_score_threshold integer, p_config_question_count integer, p_config_min_pr_size integer, p_merged_prs jsonb, p_participants jsonb, p_config_comprehension_depth text DEFAULT 'conceptual'::text)
 RETURNS uuid
 LANGUAGE plpgsql
