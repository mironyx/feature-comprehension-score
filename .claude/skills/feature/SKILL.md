...
**Before running `/diag`:** Open all changed source files in VS Code so the extension generates fresh diagnostics. The extension only exports diagnostics for files that are open in the editor — without this step, the diagnostics file may be stale or missing.

```bash
code src/path/to/file1.ts src/path/to/file2.ts   # open all changed files
...
3. After fixing, open the fixed files in VS Code again and wait 5 s, then re-run `/diag` to confirm the findings are gone — do not assume a fix worked without seeing the updated diagnostics.
