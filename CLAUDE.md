...
- `/requirements` — Transform discovery output or a freeform brief into a structured requirements document with epics, prioritised user stories, and testable acceptance criteria. Use after `/discovery` (or standalone for smaller projects) and before `/kickoff`.
