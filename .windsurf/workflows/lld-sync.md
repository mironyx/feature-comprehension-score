...
The user provides the issue number (e.g., `52`). If omitted, infer from the current branch name
or recent commits.
...
1. Determine the issue number from the user's message or from:
   ```powershell
   git log --oneline -10
   ```
   Look for `#N` references.

2. Read the issue body:
   ```powershell
   gh issue view <number>
   ```
   Extract: LLD reference (e.g., `§2.2 Task 3`), acceptance criteria, BDD specs.

3. Identify which LLD file covers this issue — look for `docs/design/lld-phase-*.md` or `docs/design/lld-*.md`. Read the relevant section.
...
2. Add a callout where the spec was materially wrong:
   ```
...
If there are no Corrections or Additions, say so explicitly — this is
