...
| **CI (GitHub Actions)** | Repository-level **Actions secret** `GITHUB_APP_PRIVATE_KEY` in `mironyx/feature-comprehension-score` only. | Only exposed to workflows on protected branches; not available to workflows from forks. | Same PEM encoding. |
...
**"CI" in this table means *our* CI only** — the GitHub Actions workflow in `mironyx/feature-comprehension-score` that runs our tests and deploys our Cloud Run service. **Customers never receive, store, or handle this key under any circumstance.** PRCC runs entirely server-side on our Cloud Run: GitHub sends us a `pull_request.*` webhook, we mint an installation token from the key in Secret Manager, fetch the PR, run the check, and write a GitHub Check Run back. The Check Run appears in the customer's PR UI alongside their own CI jobs, but it is not executed by the customer's CI — no workflow file, no action, no secret on their side. This is why we chose the GitHub App + Check Run model rather than shipping PRCC as a distributable GitHub Action.
...
## 9. Decisions and Forward Notes

Previously labelled "Open Questions". Each bullet below is **resolved** — either a decision recorded here or a forward-looking note with no design gap. Nothing in this section blocks #186.

1. **Decision — Octokit lifetime: per-request, not per-call.** The M1 helper constructs one `Octokit` per FCS assessment request or per webhook delivery (not per GitHub API call). Installation tokens change hourly, so the natural grain is the request. A single `triggerRubricGeneration` that makes diff + files + contents + issues calls creates one Octokit at the top and reuses it across all of them — the Octokit is a vehicle for the token, and the token is already cached behind it. Cheap allocation, clean ownership.
2. **Decision — `installation_id` drift on uninstall/reinstall is tolerated at V1 scale.** Current webhook handler sets `status='inactive'` on delete. On a fresh `installation.created` with the same `github_org_id`, `handle_installation_created` updates the existing row's `installation_id` and reactivates it (verified in [handle_installation_created](../../supabase/schemas/functions.sql)). **Interaction with the denormalised `assessments.installation_id`:** existing assessments created under the old installation still carry the old id. When the background rubric worker tries to mint a token for a deactivated installation, it fails cleanly (GitHub returns 404), the assessment transitions to `rubric_failed`, and the user can retrigger after reinstall. A migration to rewrite stale `installation_id` values on reinstall is *not* needed at V1 scale. Flagged for the drift scan to revisit if uninstall/reinstall traffic becomes non-trivial.
3. **Forward note — PRCC webhook route.** Not yet implemented. When it lands, it will share the same installation-token cache and receive `installation_id` via edge **E1** (verified webhook payload), consistent with §4.3. No design gap; no action required by this HLD.
4. **Decision — §4.3 principle stays in the HLD, not a standalone ADR, for now.** The cross-org isolation principle is significant enough to deserve its own ADR ("Installation-ID handling — three entry points and trust-by-construction") but is tightly coupled to ADR-0020. **Rule:** extract to a standalone ADR if and only if a second use case beyond FCS + PRCC appears that needs the same discipline. Until then, the HLD is the reference.
...
| 2026-04-08 | LS / Claude | Clarified §6.1: "CI" means our GitHub Actions workflow only. Added explicit note that customers never hold the private key — PRCC runs server-side on our Cloud Run as a GitHub Check Run, not as a customer CI job. |
| 2026-04-08 | LS / Claude | Renamed §9 "Open Questions" → "Decisions and Forward Notes". Each bullet was already answered in-place; the old name misled readers into thinking the HLD had unresolved issues. Marked each as **Decision** or **Forward note** explicitly. |
