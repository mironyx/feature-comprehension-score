...
| 2026-04-21 | LS / Claude | §17.1c sync (issue #280): all five terminal `malformed_response` paths in `runToolLoop` now produce `retryable: true` (previously `false`). The retry endpoint runs a full pipeline re-run, so fresh LLM attempts can succeed on structural failures. Aligns tool-loop with the structured generation path in `client.ts` which already used `retryable: true`. |
...
> **Implementation note (issue #280):** all five terminal `malformed_response` paths in `runToolLoop` — JSON parse failure, schema validation failure, empty final content, missing assistant message, and loop-turn-cap exhaustion — produce `retryable: true`. The retry endpoint (§E18 `/api/assessments/[id]/retry-rubric`) runs a full pipeline re-run, so a fresh LLM attempt can succeed even on these structural failures. This matches the structured generation path in `client.ts`, which already flagged the same error codes as retryable.
