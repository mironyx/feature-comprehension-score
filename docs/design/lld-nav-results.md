...
| T4 | #441 | Org overview project column + filter; project dashboard reuses table | FE | ~130 | 2 |

T1–T3 are Wave 1. T4 depends on V11 E11.1–E11.4 (projects + `assessments.project_id` non-null for FCS).

---

### §4 — V11 fix: `AssessmentOverviewTable` extension (issue #441)

Design lives in [lld-v11-e11-2-fcs-scoped-to-projects.md §B.9](lld-v11-e11-2-fcs-scoped-to-projects.md#LLD-v11-e11-2-fix-441) — that LLD owns the Story 2.2 requirement (`REQ-fcs-scoped-to-projects-project-scoped-assessment-list`) that drives the change. The `AssessmentOverviewTable` component (defined in §2 above) is extended there with `showProjectColumn` and a project filter.
